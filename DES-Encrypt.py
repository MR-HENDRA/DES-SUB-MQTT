...
def validate_input(s):
    if len(s) != 8:
        print("Error: Plaintext harus tepat 8 karakter ASCII.")
        return False
    if not all(ord(c) < 128 for c in s):
        print("Error: Plaintext harus berupa karakter ASCII (0-127).")
        return False
    return True

# Fungsi untuk validasi input kunci
def validate_key(s):
    if len(s) != 16:
        print("Error: Kunci harus tepat 16 karakter heksadesimal.")
        return False
    try:
        int(s, 16)
        return True
    except ValueError:
        print("Error: Kunci harus berupa heksadesimal (0-9, A-F).")
        return False
...
def encrypt(pt, rkb):
    ...
    for i in range(16):
        ...
        sbox_str = sbox_substitution(xor_x)
        sbox_str = pbox_permutation(sbox_str)
        left = xor(left, sbox_str)
        if i != 15:
            left, right = right, left
    return permute(left + right, tabel_iip, 64)
...
def main():
    ...
    ct_hex = bin2hex(ct)
    print("Cipher Text (Binary):", ct)
    print("Cipher Text (Hex):", ct_hex)
